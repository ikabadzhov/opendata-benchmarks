...
then
	...
	perf record -o "reports/${query}/${namerep}.data" -F 99 --call-graph dwarf ./cmpl ${cores} ${FILEN} ${query}
	rm cmpl
else
	perf record -o "reports/${query}/${namerep}.data" -F 99 --call-graph dwarf root -l -b -q "${query}/rdataframe_jitted.C(${FILEN},${cores})"
fi

perf script -i "reports/${query}/${namerep}.data" | ~/FlameGraph/stackcollapse-perf.pl > /dev/null ~/perf_reps/out.perf-folded
